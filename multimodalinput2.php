...
<head>
<title>Send Image and Text to PHP (AJAX)</title>
<link rel="stylesheet" href="multimodel.css">
</head>
<body>
...
<form id="myForm">
    <textarea name="text" placeholder="Enter text to analyze" class="card" id="transcript"></textarea><br>
    <input type="file" name="image" accept="image/jpeg" class="highlight" >
    <button class="button99" type=button
    id="send-to-gemini" onclick="submitForm()">
...
<button class="button1" id="start-record">
  ...
  function submitForm() {
    ...
    xhr.open("POST", "multimodalresponse2.php");
    xhr.onload = function() {
      if (xhr.status === 200) {
        document.getElementById("gemini-response").innerHTML = xhr.responseText;
      } else {
        document.getElementById("gemini-response").innerHTML = "Error: " + xhr.statusText;
      }
    };
    xhr.send(formData);
  }
  ...
  <script>
    ...
    const textInput = document.getElementById('gemini-response');
